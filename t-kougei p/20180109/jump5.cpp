// 左右移動＋Zキーの連射対策＋ジャンプ（ジャンプ中の左右移動制限）＋重力＋垂直抗力＋ジャンプ中の絵を変える
#include "DxLib.h"
struct SIZE2D{
	int width;
	int height;
...
int WINAPI WinMain(HINSTANCE hI, HINSTANCE hP, LPSTR lpC, int nC)
{
	...
	int keyZNow, keyZOld, txtColor, bgColor, image;
	...
	player.imageJump=LoadGraph(".\\media\\pacman_jump.bmp");
	player.isLeft=TRUE;
	...
	while(ProcessMessage()==0){
		...
		if(player.onGround)
			...
			image=player.imageJump;
		// 描画系の処理
		DrawBox(0, 0, 640, 480, bgColor, TRUE);
		DrawRotaGraph2((int)player.pos.x, (int)player.pos.y, player.imageSize.width/2, player.imageSize.height/2, 1.0, 0.0, image, TRUE, player.isLeft);
